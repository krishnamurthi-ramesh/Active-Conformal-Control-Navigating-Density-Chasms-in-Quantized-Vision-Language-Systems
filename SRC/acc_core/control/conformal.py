...
class ConformalSafetyGate:
    ...
    def calibrate(self) -> float:
        ...
        # --- 1. Drift-score threshold (standard conformal + Gaussian guardrail) ---
        scores = np.sort(self.calibration_scores)
        n = len(scores)

        # Quantile threshold (Standard Conformal)
        q_idx = int(np.ceil((n + 1) * (1 - self.epsilon)))
        q_idx = min(q_idx, n - 1)
        quantile_lambda = float(scores[q_idx])

        # Gaussian Guardrail (μ + 2.3σ) — Ensures robustness to distribution shift
        mu_w = float(np.mean(scores))
        sigma_w = float(np.std(scores))
        gaussian_lambda = mu_w + 2.3 * sigma_w

        # Conservative maximum to ensure formal coverage and robustness
        self.lambda_star = max(quantile_lambda, gaussian_lambda, self.min_threshold)
